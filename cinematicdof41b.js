...
// Step 4.1B — True Depth-Aware Near DOF
//
// Reference goal:
// - focus stays on the fixed/right half
// - the moving foreground half becomes progressively defocused as it comes closer
// - blur strength is driven by real scene depth, not a 2D panel mask
// - the hinge side stays relatively sharp while the outer edge can become strongly blurred
// - far/background content stays sharp (near-DOF only)
// - Closed and Open return to sharp through an aperture gate
//
// This module wraps the Step 3.8.1 renderer patch. The post scene is explicitly
// marked so the motion-blur module can skip it.
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Poisson-like isotropic bokeh samples. The center remains present so
        // the foreground reads as optical defocus rather than translucent ghosting.
        vec4 c = texture2D(tColor, vUv) * 0.16;
        ...
        // Overlay only where the current pixel is actually out of focus.
        // The already-rendered base (including shell motion blur) remains underneath.
...
THREE.WebGLRenderer.prototype.render = function depthAwareNearDofRender(scene, camera) {
  ...
  // First render the production base through Step 3.8.1.
  ...
  try {
    ...
    // a color + depth target. This preserves the actual fold deformation in depth.
    ...
    // Composite only the near-defocused pixels. The fixed half and background
    // remain the already-rendered sharp base.
...
console.info('Step 4.1B true depth-aware near DOF ready', {
