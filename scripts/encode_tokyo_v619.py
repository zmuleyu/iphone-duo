...
VIDEO = ART / 'iphone-duo-tokyo-v6.19-silent-review.mp4'
...
def main():
    ...
    subprocess.run(['ffmpeg','-y','-v','error','-framerate','60','-start_number','0',
        ...
        '-movflags','+faststart','-metadata','title=iPhone Duo Tokyo v6.19 fixed-step silent review',str(VIDEO)],check=True)
    ...
    subprocess.run(['ffmpeg','-y','-v','error','-i',str(VIDEO),'-vf',
        "select='eq(n,12)+eq(n,72)+eq(n,91)+eq(n,107)+eq(n,126)+eq(n,143)+eq(n,191)+eq(n,347)',scale=640:360,tile=2x4",
