"""Master-pair smoke on v5.0: continuity + full lock with the canonical assets."""
import asyncio
import base64
import json
...
class CDP:
    ...
    async def connect(self):
        ...
        self.ws = await websockets.connect(v["webSocketDebuggerUrl"], max_size=64 * 1024 * 1024)
...
async def main():
    ...
    # continuity regression with master pair (pure A world)
    await cdp.evaluate("window.__duo.setStages(0,0,0)")
    await cdp.evaluate("window.__duo.setAngle(126)")
    await asyncio.sleep(0.5)
    await cdp.shot("master_regress_70")
    # mid-transition leak frame
    await cdp.evaluate("window.__duo.setAngle(100); window.__duo.setStages(0.5,0,0)")
    await asyncio.sleep(0.4)
    await cdp.shot("master_leak")
    # full lock
    await cdp.evaluate("window.__duo.setStages(1,1,1)")
    await asyncio.sleep(0.4)
    await cdp.shot("master_lock")
